...
public function getUsageArray($usageDataFile){
    ...
    public function getGenerationProductionAray($solarSize, $address, $arrResultUsageFinal)
    {
        ...
        if (empty($buffer)) {
           ...
           //_DX($arrResultUsageFinal);
            ...
            $arrGenerationFinal = $this->checkStartMonthMiddleDay($arrGenerationFinal);
    ...
    public function checkStartMonthMiddleDay($arrGenerationFinal){
        
        if(!is_array($arrGenerationFinal) || empty($arrGenerationFinal)){
            return false;
        }
        
        
        if($arrGenerationFinal[1][3] > 1){
            $totalCount = count($arrGenerationFinal);
            $startYear = $arrGenerationFinal[1][8];
            $startMonth = $arrGenerationFinal[1][2];
            $startDay = $arrGenerationFinal[1][3];
            $totalDaysInStartMonth = cal_days_in_month(CAL_GREGORIAN,$startMonth,$startYear);
            $processedStartMonthDaysInStartYear = $totalDaysInStartMonth - $startDay + 1;
            $remainingStartMonthDaysInEndYear = $totalDaysInStartMonth - $processedStartMonthDaysInStartYear;
            $remainingIntervalCount = $remainingStartMonthDaysInEndYear*24*4;
            $firstElements = array_slice($arrGenerationFinal,-$remainingIntervalCount);
            $lastElements =  array_slice($arrGenerationFinal,1,$totalCount-($remainingIntervalCount+1));
            $firstElement = array_slice($arrGenerationFinal,0,1);
            
            
            $finalGenerationArray = array_merge($firstElement,$firstElements,$lastElements);
            
            return $finalGenerationArray;

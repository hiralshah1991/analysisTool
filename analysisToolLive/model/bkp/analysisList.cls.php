    ...
    public function getAnalysisDetail(){
       ...
       if(!$this->aid || !is_numeric($this->aid)){
           ...
           return false;
            ...
            $sql = "SELECT a.*, r.rateschedule_name, r.lower_threshold, r.upper_threshold FROM analyses as a LEFT JOIN rateSchedule as r ON a.rate_schedule = r.rsid WHERE analysis_id = ".$this->aid;
            $res = $this->conn->query($sql);
            $result = $this->getArray($res);

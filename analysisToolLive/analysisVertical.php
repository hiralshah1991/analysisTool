                    ...
                    if (isset($arrToProcess) && ! empty($arrToProcess)) {
                        ...
                        foreach ($batterySizeArray as $batterySize) {
                            ...
                            foreach($arrToProcess as $k => $v) {
                                ...
                                if(($totalDaysOFMonth == $day && $hour == 23.45) || ($k == $lastKey && $hour = 23.45 && $year == $endYear)){
                                   ...
                                   /* _D("Battery Size = ".$batterySize);
                                    _D("Solar Size = ".$solarSizeText);*/
                                    if($NBNS){
                                    ...
                                    $upperThresholdCross[$month] = $upperThresholdCount;
                                        ...
                                        //This condition is applied when year's starting date is not the first date of month
                                        /*_D("solarSizeText = ". $solarSizeText);
                                        _D("batterySizeText = ". $batterySizeText);
                                        _D("year = ".$year);
                                        _D("month = ".$month);
                                        _D("Day = ".$day);
                                        _D($annualData[$solarSizeText][$batterySizeText][$month]);
                                        _D("*****");
                                       if(is_array($annualData[$solarSizeText][$batterySizeText][$month]) && !empty($annualData[$solarSizeText][$batterySizeText][$month])){
                                        _D("inif");
                                        _D("_______");
                                        //_D($annualData[$solarSizeText][$batterySizeText][$month]);
                                       
                                        $previousSolarGeneration = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['totalSolarGeneration'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalSolarGeneration+=$previousSolarGeneration;
                                        
                                        
                                        $previousEnergyFromGrid = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['totalEnergyFromGrid'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalEnergyFromGrid+=$previousEnergyFromGrid;
                                        
                                        $previousMonthlyBillCost = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['totalBill'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalMonthlyBillCost[$month]+=$previousMonthlyBillCost;
                                        
                                        $previousEnergyCharges = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['energyCharges'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalEnergyCharges+=$previousEnergyCharges;
                                        
                                        $previousDemandCharge = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['demandCharges'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalDemandChargeMonth+=$previousDemandCharge;
                                        
                                        $previousCriticalPeakCharges = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['criticalPeakCharges'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalCriticalPeakCharges+=$previousCriticalPeakCharges;
                                        
                                        $previousCPPIncentiveAmount = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['CPPIncentive'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $CPPIncentiveAmount+=$previousCPPIncentiveAmount;
                                        
                                        $seasonFixedBillCheck = $season."FixedChargeBillCyclePerDay";
                                        if($$seasonFixedBillCheck){
                                            $previousfixedChargeValue = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['fixedCharges'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                            $fixedChargeValue+=$previousfixedChargeValue;
                                        }
                                        
                                        $previousEnergyExport = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['totalEnergyExport'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalEnergyExport+=$previousEnergyExport;
                                        
                                        $previousIncomeEE = filter_var($annualData[$solarSizeText][$batterySizeText][$month]['totalIncomeEE'],FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                        $totalIncomeEE+=$previousIncomeEE;
